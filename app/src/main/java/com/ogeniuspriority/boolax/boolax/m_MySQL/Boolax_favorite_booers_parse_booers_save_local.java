 ...
 */
public class Boolax_favorite_booers_parse_booers_save_local extends AsyncTask<Void, Void, Boolean> {
    Context c;
    String jsonData;
    ...
    public Boolax_favorite_booers_parse_booers_save_local(Context c, String jsonData, ListView boolax_boos_list) {
        this.c = c;
    ...
    protected void onPostExecute(Boolean isParsed) {
        ...
        if (isParsed) {
            ...
            //--------------------
            // boolax_boos_list.setAdapter(new Boolax_favorite_booers_custom_adapter_for_booers(c, My_favorite_boolax_boos));
            //-----------
            if (My_favorite_boolax_boos.size() > 0) {
                Toasty.success(c, "New Booers to consider!", Toast.LENGTH_SHORT, true).show();
                ...
                Toasty.info(c, "No new Booers!", Toast.LENGTH_SHORT, true).show();
                NDS_boolax_db_adapter thDb;
                thDb = new NDS_boolax_db_adapter(c);
                thDb.openToWrite();
                Cursor allUsersSaved = thDb.GET_ALL_MYDATABASE_NDS_TABLE_BOOLAX_BOOERS();
    ...
    private Boolean parseData() {
        try {
            ...
            for (int counter = 0; counter < Boollax_favorite_booers_boos_loaded.length(); counter++) {
                ...
                try {
                    if (thDb.save_MYDATABASE_NDS_TABLE_BOOLAX_BOOERS_locally(fname, lname,
                            ...
                            avatar_image, remote_id,
                            interested_in, your_weight, your_height,
                            skin_color)) {
                        Log.w("saved_", "saved okay!");

                    }
                } catch (Exception vbcv) {
