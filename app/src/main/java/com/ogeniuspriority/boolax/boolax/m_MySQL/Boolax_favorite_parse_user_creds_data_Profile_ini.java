...
import com.ogeniuspriority.boolax.boolax.boolax_basic_profile;
 ...
 */
public class Boolax_favorite_parse_user_creds_data_Profile_ini extends AsyncTask<Void, Void, Boolean> {
    Context c;
    ...
    public Boolax_favorite_parse_user_creds_data_Profile_ini(Context c, String jsonData) {
        this.c = c;
            ...
            if (new database_enablers(c).findIfUserCreated()) {
                if (db.save_MYDATABASE_NDS_TABLE_BOOLAX_USER_locally(fname[0], lname[0],
                    ...
                    if (db.save_MYDATABASE_NDS_TABLE_BOOLAX_ACCOUNT_LOG_locally("1", remote_id[0],
                            regdate[0])) {
                        Toasty.success(c, "Profile uploaded!", Toast.LENGTH_SHORT, true).show();
                        //------------
                        Glide.with(c)
                                .load(Config.BOOLAX_FAVORITE_THE_USERS_AVATARS.toString() + boo_avatar_image[0])
